...
import cv2
import time
from control_game import PressKey, ReleaseKey, A, S, D, W
import pyautogui
...
def process_image(original_image):
    ...
    processed_image = cv2.Canny(processed_image, 100, 300)
    processed_image = cv2.GaussianBlur(processed_image, (5,5), 0)
    vertices = np.array([[10,500],[10,300],[300,200],[500,200],[800,300],[800,500],
                         ], np.int32)
    processed_image = roi(processed_image,[vertices])
    lines = cv2.HoughLinesP(processed_image, 1, np.pi/180, 100, np.array([]), minLineLength=150, maxLineGap=15)
    m1 = 0
    m2 = 0
    try:
...
while(True):
    ...
    processed_image, original_image, m1, m2 =  process_image(printscreen_pil)
    #draw_lines(roi_image, lines)
    cv2.imshow('window',processed_image)
    cv2.imshow('window2',cv2.cvtColor(np.array(original_image), cv2.COLOR_BGR2RGB))
    #cv2.cvtColor(np.array(printscreen_pil), cv2.COLOR_BGR2RGB))
    if m1 > 0 and m2 > 0:
        left()
        print('left')
    elif(m1 < 0 and m2 < 0):
